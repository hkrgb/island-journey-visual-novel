...
const resolveUrl=v=>v&&v.indexOf('firestore:')===0?(mediaCache[v.slice(10)]||''):(v||'');
const assetUrl=(type,key)=>resolveUrl((assets[type]&&assets[type][key])||(type==='bg'?BASE_BGS:BASE_SPR)[key]||'');
const uid=p=>p+'-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,6);
const lineDefault=()=>({sp:'旁白',en:'',sprite:'',side:'',t:'新對白',choices:[]});
const sceneDefault=c=>({id:uid('scene'),c,name:'新場景',bg:'ferry',backgroundType:'image',place:'',start:false,lines:[lineDefault()]});
const payload=()=>({version:4,type:'full',chapters,story:scenes.map((s,i)=>({s,i})).sort((a,b)=>a.s.c-b.s.c||a.i-b.i).map(x=>x.s),settings,assets,library,characters});
function setStatus(t){$('#status').textContent=t;$('#count').textContent=chapters.length+' 章 · '+scenes.length+' 個場景'+(dirty?' · ⚠ 有未儲存變更':'')}
...
function showChapterForm(){mode='chapter';hideForms();$('#chapterForm').hidden=false;const c=chapters[openChapter];$('#chapterId').value=c.id||'';$('#chapterNo').value=c.no||'';$('#chapterName').value=c.name||'';$('#chapterMusic').value=c.music||'';$('#chapterIntro').value=Array.isArray(c.intro)?c.intro.join('\n'):(c.intro||c.video||'');$('#chapterOutro').value=Array.isArray(c.outro)?c.outro.join('\n'):(c.outro||'')}
const choiceActions=[['goto','前往指定場景'],['money','增加／扣除金錢'],['score','增加／扣除分數'],['affection','增加／扣除好感度'],['sprite','改變角色立繪'],['iframe','開啟外部遊戲／網頁'],['next','前往下一段']];
...
function cleanMediaUrl(value){const s=String(value||'').trim().replace(/^['"]|['"]$/g,''),md=s.match(/\[[^\]]*\]\((https?:\/\/[^)]+)\)/i);return(md?.[1]||s).trim()}
function syncChapter(){if(mode!=='chapter')return;const c=chapters[openChapter];c.id=$('#chapterId').value;c.no=$('#chapterNo').value;c.name=$('#chapterName').value;c.music=$('#chapterMusic').value;c.intro=$('#chapterIntro').value.split('\n').map(cleanMediaUrl).filter(Boolean);c.outro=$('#chapterOutro').value.split('\n').map(cleanMediaUrl).filter(Boolean);$('#chapterIntro').value=c.intro.join('\n');$('#chapterOutro').value=c.outro.join('\n');refreshSelects();mark('篇章已修改');tree()}
function syncScene(){if(mode!=='scene'||selectedScene<0)return;const s=scenes[selectedScene];s.id=$('#sceneId').value;s.c=+$('#sceneChapter').value;s.name=$('#sceneName').value;s.backgroundType=$('#backgroundType').value;s.bg=$('#bg').value;s.place=$('#place').value;s.street={url:$('#streetUrl').value,lat:$('#streetLat').value,lng:$('#streetLng').value,heading:$('#streetHeading').value,pitch:$('#streetPitch').value,zoom:$('#streetZoom').value};s.iframeUrl=$('#iframeUrl').value;s.requirements=$('#requirements').value;s.effects=$('#effects').value;s.music=$('#sceneMusic').value;s.start=$('#chapterStart').checked;openChapter=s.c;toggleStreetFields();mark('場景已修改');tree();visual()}
function analyseStreetUrl(){const raw=$('#streetUrl').value.trim();if(!raw)return alert('請先貼上 Google Maps 街景 URL');let decoded;try{decoded=decodeURIComponent(raw)}catch{decoded=raw}const at=decoded.match(/@(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?),(?:\d+(?:\.\d+)?a,)?(-?\d+(?:\.\d+)?)y,(-?\d+(?:\.\d+)?)h,(-?\d+(?:\.\d+)?)t/),ll=decoded.match(/[?&](?:cbll|query)=(-?\d+(?:\.\d+)?)[,%2C]+(-?\d+(?:\.\d+)?)/i),plain=decoded.match(/@(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)/);const pos=at||ll||plain;if(!pos)return alert('未能分析此 URL。請在 Google Maps 開啟街景後，再複製瀏覽器完整網址。');$('#streetLat').value=pos[1];$('#streetLng').value=pos[2];if(at){$('#streetZoom').value=Math.max(20,Math.min(120,+at[3]||90));$('#streetHeading').value=at[4]||0;$('#streetPitch').value=(+at[5]||90)-90}syncScene();setStatus('街景 URL 已分析並填入鏡頭資料')}
async function compress(file){return new Promise((resolve,reject)=>{const im=new Image(),reader=new FileReader();reader.onerror=reject;reader.onload=()=>im.src=reader.result;im.onerror=reject;im.onload=()=>{const max=1400,scale=Math.min(1,max/Math.max(im.width,im.height)),canvas=document.createElement('canvas');canvas.width=Math.round(im.width*scale);canvas.height=Math.round(im.height*scale);canvas.getContext('2d').drawImage(im,0,0,canvas.width,canvas.height);const data=canvas.toDataURL(file.type==='image/png'?'image/webp':'image/jpeg',.82);if(data.length>850000)reject(new Error('圖片壓縮後仍太大，請使用較細的圖片'));else resolve(data)};reader.readAsDataURL(file)})}
async function upload(file,name,type){if(!file||!name)throw new Error('請選擇圖片及填寫素材名稱');setStatus('正在壓縮及上載圖片…');const dataUrl=await compress(file),id=(type+'-'+name).replace(/[^a-zA-Z0-9_-]+/g,'-');await setDoc(doc(db,'media',id),{dataUrl,type,name,updatedAt:serverTimestamp(),updatedBy:auth.currentUser.email});mediaCache[id]=dataUrl;assets[type][name]='firestore:'+id;refreshSelects();mark();return name}
async function loadProjects(){const snap=await getDocs(collection(db,'projects'));projects=snap.docs.map(d=>({id:d.id,...d.data()}));if(!projects.some(p=>p.id==='island-journey'))projects.unshift({id:'island-journey',name:'離島旅程',cover:'../assets/official/cover-island.jpg',legacy:true});if(!projects.some(p=>p.id==='media-demo'))projects.push({id:'media-demo',name:'轉場與360功能示範',cover:'../assets/official/cover-island.jpg',demo:true});renderProjects()}
...
function exportFull(){downloadJson(payload(),(settings.projectName||'game')+'-完整backup.json');setStatus('已匯出完整資料')}
function exportCharacters(){downloadJson({type:'characters',characters},(settings.projectName||'game')+'-角色資料庫.json');setStatus('已匯出角色資料庫')}
function exportAssets(){downloadJson({type:'assets',library,assets},(settings.projectName||'game')+'-素材資料庫.json');setStatus('已匯出素材資料庫')}
function exportSettings(){downloadJson({type:'settings',settings},(settings.projectName||'game')+'-遊戲設定.json');setStatus('已匯出遊戲整體設定')}
function exportStory(){downloadJson({type:'story',chapters,story:scenes},(settings.projectName||'game')+'-劇本章節.json');setStatus('已匯出劇本＋章節')}
...
$('#exportFullBtn').onclick=exportFull;
$('#exportCharsBtn').onclick=exportCharacters;
$('#exportAssetsBtn').onclick=exportAssets;
$('#exportSettingsBtn').onclick=exportSettings;
$('#exportStoryBtn').onclick=exportStory;

$('#importInput').onchange=async e=>{try{const data=JSON.parse(await e.target.files[0].text());const t=data.type||'full';if(t==='full'||(!data.type&&data.chapters&&data.story)){chapters=data.chapters||[];scenes=normalizeStory(data.story||[]);settings={...DEFAULT_SETTINGS,...(data.settings||{})};assets=data.assets||{bg:{},sprite:{}};library=data.library||library;characters=data.characters||[];mark('完整資料已匯入');showSettings()}else if(t==='characters'){characters=data.characters||[];mark('角色資料庫已匯入');showCharacters()}else if(t==='assets'){if(data.library)library=data.library;if(data.assets)assets=data.assets;mark('素材資料庫已匯入');showLibrary()}else if(t==='settings'){settings={...DEFAULT_SETTINGS,...(data.settings||{})};Object.entries(sf).forEach(([k,el])=>el.value=settings[k]||'');syncDisplay(false);settingsVisual();mark('遊戲設定已匯入');showSettings()}else if(t==='story'){chapters=data.chapters||[];scenes=normalizeStory(data.story||[]);mark('劇本＋章節已匯入');tree();showSettings()}else{alert('無法識別此 JSON 類型')}e.target.value=''}catch(err){alert('匯入失敗：'+err.message)}}

$('#saveBtn').onclick=()=>save();$('#publishBtn').onclick=()=>confirm('確定發布給所有玩家？')&&save('published');$('#previewBtn').onclick=()=>{localStorage.setItem('islandAdminPreview',JSON.stringify(payload()));$('#preview iframe').src='../?preview='+Date.now()+'&game='+encodeURIComponent(currentProject);$('#preview').showModal()};$('#closePreview').onclick=()=>$('#preview').close();
$('#backProjects').onclick=showProjectHub;$('#hubLogout').onclick=$('#logoutBtn').onclick;$('#newProjectBtn').onclick=async()=>{const name=prompt('新遊戲名稱');if(!name)return;const id=slug(name)+'-'+Date.now().toString(36);await setDoc(doc(db,'projects',id),{name:name.trim(),cover:'',owner:auth.currentUser.email,createdAt:serverTimestamp()});projects.push({id,name:name.trim(),cover:''});renderProjects();await openProject(id)};
onAuthStateChanged(auth,async u=>{if(!u){$('#login').hidden=false;$('#cms').hidden=true;$('#projectHub').hidden=true;return}if((u.email||'').toLowerCase()!==ADMIN){$('#loginMsg').textContent='帳戶 '+u.email+' 沒有管理權限';await signOut(auth);return}$('#login').hidden=true;await openProject('island-journey')});
